 ...
 */
import { renderEmailHtml, renderEmailText, resolveAppBaseUrl, resolveLogoUrl } from "./emailHtml";
...
// Maps an internal sequence_type to the letter used in email_templates.sequence_key.
// post_visit → "a" (a_step_1..a_step_5), cold_outreach → "b" (b_step_1..b_step_5).
const SEQ_KEY_LETTER = { post_visit: "a", cold_outreach: "b" };

// Normalize any sequence identifier to the 'a' | 'b' letter used in sequence_key.
// Accepts either the letter directly ("a"/"b") or the sequence_type ("post_visit"/"cold_outreach").
export function seqLetter(sequence) {
  if (sequence === "a" || sequence === "b") return sequence;
  return SEQ_KEY_LETTER[sequence] || null;
}

// Build the email_templates.sequence_key for a given sequence + 1-indexed step,
// e.g. ("post_visit", 2) → "a_step_2", ("b", 4) → "b_step_4".
export function sequenceKeyFor(sequence, step) {
  const letter = seqLetter(sequence);
  return letter ? `${letter}_step_${step}` : null;
}

// ── Sequence A — post in-person visit ──
// V2 copy — transportation-as-pain-point through-line, CTA in steps 1 + 4, graceful close.
// ACCURACY: transportation is included with Community-Based Supports (CBS) ONLY —
// never state or imply it applies to ISS or Respite.
const SEQUENCE_A = [
  {
...
I wanted to leave you with one thing: if you ever have a client who needs Community-Based Supports and transportation is a barrier, we include transportation with every CBS placement. No separate authorization needed, no extra cost to the family.

If anyone on your caseload needs ISS, CBS, or Respite and you want a provider who can move quickly, I'd love the chance to help.
...
Since we met on {{visit_date}}, I've been thinking about something I hear from coordinators constantly — the hardest placements aren't the ones with complex needs. They're the ones where the family can't get the person to their community program because transportation falls through.

That's exactly why we built transportation into our Community-Based Supports from day one. No separate van company, no missed pickups. We handle it as part of the service.
...
Nate`,
  ...
  {
    subject: "We have availability right now",
    body: `Hi {{first_name}},

Quick note — we currently have availability for ISS, CBS, and Respite placements and can get started within days of receiving a referral, not weeks.

Here's what a referral to Harmony looks like in practice:

You send me the client's name and a good time to connect. I coordinate directly with the family, handle intake paperwork, and get services scheduled. For CBS clients, transportation is covered from day one. You get a responsive provider who keeps you updated without you having to chase us down.
...
Harmony is here whenever you have a client who needs ISS, CBS, or Respite with a provider who actually picks up the phone. For CBS placements, we include transportation so that's one less thing for the family to figure out. We serve all of New Jersey and accept private pay alongside DDD.
...
If you have a client in New Jersey who needs ISS, CBS, or Respite placement, I can have services started within days. For CBS clients, we include transportation at no extra cost.

My name is Nate Ojugo. I run Harmony Homecare Agency — we're a DDD-approved provider in Burlington County offering Individual Support Services, Community-Based Supports, and Respite Care.
...
I wanted to follow up with something I keep running into: families who qualify for Community-Based Supports but can't find a provider who handles transportation. The authorization is there, the funding is there, but the client can't physically get to their program. So nothing happens.

That's the gap Harmony was built to close. We include transportation with every CBS placement. It's not an add-on. It's how we operate.
...
You send me the client's name and contact. I handle outreach to the family, intake paperwork, and scheduling. For CBS, transportation is built in from day one. I keep you in the loop without you having to follow up with me.
...
Harmony Homecare is a DDD-approved provider in Burlington County offering ISS, CBS, and Respite Care. For CBS placements, transportation is included. We serve clients throughout New Jersey and accept private pay alongside DDD.
...
// Hardcoded fallback { subject, body } (pre-personalization) for a sequence step.
// Used only when the email_templates lookup fails (Supabase down / missing row),
// so the cron never breaks. The database is the source of truth in normal operation.
export function defaultStep(sequence, step) {
  const letter = seqLetter(sequence);
  const steps = letter === "b" ? SEQUENCE_B : SEQUENCE_A;
  return steps[step - 1] || null;
}

// Returns { subject, body } for a sequence row at its current step (1-indexed),
// using the HARDCODED defaults. This is the synchronous fallback path — the live
// send path uses renderStepFromDb() so that Templates-tab edits take effect.
export function renderStep(seq, step) {
  const tpl = defaultStep(seq.sequence_type, step);
  if (!tpl) return null;
  return {
...
/**
 * Fetch a sequence step's { subject, body } from the email_templates table by
 * sequence_key (e.g. "a_step_2"). This is the SINGLE SOURCE OF TRUTH — editing a
 * template in the Templates tab changes what the cron sends on its next run.
 *
 * Falls back to the hardcoded default (defaultStep) if the query errors or the
 * row is missing, so a Supabase outage never breaks the sequence sender.
 *
 * @returns {Promise<{subject:string, body:string}|null>} raw (un-personalized) content
 */
export async function fetchSequenceStepContent(supabase, sequence, step) {
  const fallback = defaultStep(sequence, step);
  const sequence_key = sequenceKeyFor(sequence, step);
  if (!supabase || !sequence_key) return fallback;
  try {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error || !data) {
      if (error) console.error(`[sequenceEmails] template lookup failed for ${sequence_key}:`, error.message);
      return fallback;
    }
    return { subject: data.subject, body: data.body };
  } catch (err) {
    console.error(`[sequenceEmails] template lookup threw for ${sequence_key}:`, err);
    return fallback;
  }
}

// DB-backed equivalent of renderStep: pulls content from email_templates by
// sequence_key, then applies merge tags. Falls back to hardcoded defaults on any
// failure. If no supabase client is supplied, defers to the sync renderStep.
export async function renderStepFromDb(supabase, seq, step) {
  if (!supabase) return renderStep(seq, step);
  const tpl = await fetchSequenceStepContent(supabase, seq.sequence_type, step);
  if (!tpl) return null;
  return {
    subject: personalize(tpl.subject, seq),
    body: personalize(tpl.body, seq),
...
// Body → HTML conversion (escape + linkify, preserving embedded <img> tags) is
// shared via lib/emailHtml.js — see the top-of-file import.

const SIGNATURE_FOOTER = (logoUrl) => `
...
// Plain-text fallback footer (mirrors SIGNATURE_FOOTER + CANSPAM_FOOTER).
const PLAIN_FOOTER = `Nate Ojugo
Harmony Homecare Agency, LLC
...
export function buildHtml(bodyText) {
  ...
  return renderEmailHtml(bodyText, { footerHtml: `${SIGNATURE_FOOTER(logoUrl)}\n  ${CANSPAM_FOOTER}` });
}

export function buildText(bodyText) {
  return renderEmailText(bodyText, PLAIN_FOOTER);
}

// Brochure PDF attached to every sequence email — served from public/brochure.pdf.
// Uses the stable production domain (NOT process.env.VERCEL_URL — see
// resolveAppBaseUrl for why that silently breaks the attachment for recipients).
function resolveBrochureUrl() {
  return `${resolveAppBaseUrl()}/brochure.pdf`;
}
 ...
 *
 * @param supabase Supabase client used to pull step content from email_templates
 *   by sequence_key. If omitted, falls back to the hardcoded default content.
 * @param override Optional { subject, body } to use instead of the template
 *   (already merge-filled) — used for per-contact email overrides (highest priority).
 * @returns {Promise<{ok:boolean, subject:string, resendId?:string, error?:string}>}
 */
export async function sendSequenceStep({ supabase, resendKey, from, seq, step, override }) {
  // Lookup order (Feature 1 #6): per-contact override first, otherwise the template
  // content from email_templates (DB source of truth), falling back to hardcoded defaults.
  const rendered = override
    ? { subject: personalize(override.subject, seq), body: personalize(override.body, seq) }
    : await renderStepFromDb(supabase, seq, step);
  ...
  const text = buildText(rendered.body);
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html,
        text,
        attachments: [{ filename: "Harmony-Homecare-Brochure.pdf", path: resolveBrochureUrl() }],
      }),
    });
