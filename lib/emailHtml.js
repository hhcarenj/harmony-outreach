 ...
 *
 * Served from Supabase Storage, NOT the dashboard's own domain. The dashboard is
 * now access-controlled (staff login), and anything that gates it — an auth
 * redirect, or Vercel Deployment Protection if it's ever switched on — would also
 * gate /logo.png and silently break the logo in every outbound email. That's the
 * bug fixed in e14583f, reintroduced by the security work if we left it alone.
 * Storage is a separate, permanently public host with no such coupling.
 *
 * Defined here (the leaf module) so all three senders — weekly batch, sequences,
 * and one-off campaign sends — resolve it identically.
 */
export const STORAGE_LOGO_URL =
  "https://skpwjwluxfkivxnlfpqb.supabase.co/storage/v1/object/public/email-images/brand/logo.png";

export function resolveLogoUrl() {
  return process.env.LOGO_URL || STORAGE_LOGO_URL;
}
